...
def layer_norm_kernel(
    ...
    input = tl.load(input_ptrs, mask=batch_mask[:, None] & feat_mask[None, :])

    mean = tl.sum(input, axis=-1, keepdim=True) / dim
    diff = input - mean
    var = tl.sum(diff * diff, axis=-1, keepdim=True) / dim
    output = input / tl.sqrt(var + eps)
...
def compare_result():
    # 随机生成一组输入数据
    x = torch.randn(10, 16, 32, device="cuda", requires_grad=False)
    dim = x.shape[-1]
    eps = 1e-5
    ...
    print(f"allclose (atol=1e-4, rtol=1e-4): {allclose}")

    # 如果希望严格一点，可以直接断言
    assert allclose, "pytorch_layer_norm 和 triton_layer_norm 输出不一致！"
